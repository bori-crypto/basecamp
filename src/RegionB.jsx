import React, { useState, useEffect, useContext } from 'react'; // ✅ useContext 유지
import { 
  Camera, 
  Bike, 
  Footprints, 
...
import RunningLog from './RegionB/RunningLog'; // ✅ 러닝 로그 컴포넌트 유지
import GearManagement from './RegionB/GearManagement'; // ✅ 자동화 화면 컴포넌트 임포트 (필수)
import { AppContext } from './App'; // ✅ 서버 주소 및 암호 가져오기용 유지
...
  // ✅ App.jsx에서 관리 중인 러닝 워커 주소와 관리자 암호 가져오기 유지
...
        { label: '장비 관리', detail: ['신발 마일리지', '워치 설정'] } // ✅ '장비 관리' 클릭 시 자동화 화면으로 연결 예정
      ]
    },
    fuel: {
      label: '주유 기록',
...
        {step === 0 ? (
          /* 1단계: 유니버스 메인 (오빠의 원본  레이아웃 및 효과 완벽 보존) */
...
                >
                   {menuData[key].icon} {/* ✅ 원본 아이콘 규격 유지 */}
                </div>
...
              /* ✅ Step 1 보완: '장비 관리' 클릭 시 자동화 화면으로 교체 (정밀 수리) */
              <GearManagement 
                isAdmin={isAdmin} 
                workerUrl={RUNNING_WORKER_URL} 
                adminPassword={adminPassword} 
...
                        {item === '신발 마일리지' && <Sparkles size={14} className="text-amber-400 animate-pulse" />} {/* ✅ 신발 마일리지 표시 (선택사항) */}
                      </div>
