...
export const BikeRouteFullMapView = ({ title }) => {
  ...
  const handleSearchRoutePreview = async () => {
    ...
    setIsRouting(true);
    try {
      // 텍스트 수동 입력 대응: 좌표 캐시 없으면 지오코딩 수행
      ...
      const res = await fetch(directionUrl, {
        method: 'POST',
      ...
      if (!res.ok) {
        alert(`네이버 서버 에러: ${data.detail || '인증 실패 또는 권한 부족'}\n(워커에서 [Deploy] 버튼을 눌렀는지 꼭 확인해 줘!)`);
        setIsRouting(false); return;
      }

      const routePath = data.route.traavoidcaronly?.[0]?.path || data.route.traoptimal?.[0]?.path;
      if (!routePath) { alert("이 좌표로는 도로를 찾을 수 없어! 마커를 도로 쪽으로 더 옮겨봐."); setIsRouting(false); return; }
  ...
  const handleSave = async () => {
    try {
      ...
      const res = await fetch(BIKE_WORKER_URL, {
        ...
        body: JSON.stringify({ ...routeData, waypoints: finalWaypoints })
      });
      if (res.ok) { alert('저장 완료! 💾'); setIsEditing(false); } else alert('저장 권한 없음');
  ...
  return (
    <div className="relative w-full h-full overflow-hidden rounded-[2.5rem] text-slate-100 animate-in fade-in duration-700 font-sans shadow-2xl bg-[#0f172a] border border-white/10" onContextMenu={(e) => e.preventDefault()}>
      ...
      {isMapEngineMissing && <div className="absolute inset-0 z-[1] flex items-center justify-center bg-black/40 backdrop-blur-sm"><div className="text-center animate-pulse"><AlertTriangle size={48} className="mb-4 text-amber-500/80 mx-auto" /><p className="text-slate-300 font-black tracking-widest uppercase text-sm">엔진 연결 대기 중...</p></div></div>}
            ...
            <div className="flex items-center gap-2 shrink-0 ml-3">
              {isPrivateMode && !isCollapsed && !isEditing && (
                <div className="flex gap-1 mr-1" onClick={e => e.stopPropagation()}>
                  <button onClick={handleDeleteDetail} className="p-2 bg-red-500/10 hover:bg-red-500/20 rounded-lg border border-red-500/20"><Trash2 size={14} className="text-red-400"/></button>
                  <button onClick={() => setIsEditing(true)} className="p-2 bg-white/5 hover:bg-white/10 rounded-lg border border-white/10"><Edit2 size={14} className="text-slate-300"/></button>
                </div>
              )}
              ...
              <>
                <div className="flex flex-col gap-2 relative">
                  <div className="flex items-center gap-3">
                    ...
                    <input placeholder="출발지 (지도 우클릭)" value={startPoint} onChange={e => { setStartPoint(e.target.value); coordsRef.current.start = null; }} className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm outline-none text-white font-medium"/>
                  </div>
                  {waypointPoints.map((wp, idx) => (
                    <div key={idx} className="flex items-center gap-3 pl-2 border-l border-white/10 ml-2">
                      <div className="relative z-10 w-5 h-5 rounded-full bg-emerald-500 border border-white/20 flex items-center justify-center text-[10px] font-black text-white shrink-0 shadow-md">{idx + 1}</div>
                      <input placeholder={`경유지 ${idx + 1}`} value={wp} onChange={e => { const n = [...waypointPoints]; n[idx] = e.target.value; setWaypointPoints(n); coordsRef.current.waypoints[idx] = null; }} className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm outline-none text-white font-medium"/>
                      <button onClick={() => setWaypointPoints(waypointPoints.filter((_, i) => i !== idx))} className="text-red-400 p-2"><Trash2 size={16}/></button>
                    </div>
                  ))}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <input placeholder="도착지 (지도 우클릭)" value={goalPoint} onChange={e => { setGoalPoint(e.target.value); coordsRef.current.goal = null; }} className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm outline-none text-white font-medium"/>
                  </div>
                  {waypointPoints.length < 13 && <button onClick={() => setWaypointPoints([...waypointPoints, ''])} className="mt-1 flex items-center gap-2 text-xs font-bold text-emerald-400 p-2 hover:bg-white/5 rounded-xl transition-all w-max"><Plus size={14} strokeWidth={3}/> 경유지 추가</button>}
                </div>
                <button onClick={handleSearchRoutePreview} disabled={isRouting} className="mt-2 w-full bg-emerald-500/20 border border-emerald-500/50 hover:bg-emerald-500/30 disabled:opacity-50 text-emerald-400 font-bold py-3 rounded-xl text-sm transition-all shadow-lg flex items-center justify-center gap-2">
                  <Route size={18} /> {isRouting ? '위성 좌표 교신 중...' : '바이크(이륜차) 길찾기 미리보기'}
                </button>
                <textarea placeholder="메모 (노면, 공기압 등)" value={routeData.memo} onChange={e => setRouteData({...routeData, memo: e.target.value})} className="bg-white/5 border border-white/10 rounded-xl p-3 text-sm h-20 outline-none resize-none mt-2 custom-scrollbar text-slate-300" />
                <div className="flex justify-between items-center mt-2 pt-2 border-t border-white/10">
                  <label className="flex items-center gap-2 bg-white/10 px-4 py-2.5 rounded-xl text-sm font-medium cursor-pointer border border-white/10 text-slate-200"><Paperclip size={16} className="text-indigo-400"/> GPX/KML<input type="file" hidden accept=".gpx,.kml" onChange={(e) => { /* 파일첨부 로직 동일 */ }} /></label>
                  <button onClick={handleSave} className="flex items-center gap-2 bg-indigo-500 hover:bg-indigo-600 px-8 py-2.5 rounded-xl text-sm font-bold shadow-lg shadow-indigo-500/30 text-white"><Save size={16}/> 최종 저장</button>
                </div>
              </>
            ) : (
